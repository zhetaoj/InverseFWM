...
class AdjointMethodFM2D(AdjointMethod):
    ...
    def calc_gradient(self, sim, params):
        ...
        for i in range(lenp):
            ...
            dAdp = (Af[0]-Ai[0])/step #TODO: replace it with modification in FDFDs [Zhetao]
            # print('[DEBUG adjointmethod 1050]' ,  Af, Af[0]) 
            # print('[DEBUG adjointmethod 1502]' ,  np.max(np.abs(dAdp)))
            ...
            if(NOT_PARALLEL):
                ...
                if(isinstance(sim.sims[0], fdfd.FDFD_TM)):
                    dmudp = dAdp_full[0::3].reshape([M,N]) * 1j
                    depsdp = dAdp_full[1::3].reshape([M,N]) / 1j
                elif(isinstance(sim.sims[0], fdfd.FDFD_TE)):
